...
def dataset_df_row_to_JSON(row, dataset_code, page_path) -> dict:
    ...
    result_json["dataset_webpage_path"] = page_path
    ...
    result_json["dataset_keywords"] = keywords

    result_json["abstract"] = html.escape(str(row.get("abstract", "Missing abstract")))
    result_json["allow?"] = bool(row.get("allow?", "Missing").lower() in {"yes", "y", "allow", "allowed"})
